...
def eval_bleu(
    args, eval_data, eval_examples, model, tokenizer, split_tag, cur_task, criteria
):
...
def main():
    ...
    if args.do_train:
        if args.local_rank in [-1, 0] and args.data_num == -1:
            summary_fn = "./tensorboard/{}".format(
                "/".join(args.output_dir.split("/")[1:])
            )
            tb_writer = SummaryWriter(summary_fn)

        # Prepare training data loader
        train_examples_data_dict = load_and_cache_multi_gen_data(
            args, pool, tokenizer, "train", is_sample=False
        )
        train_data_list = [v[1] for k, v in train_examples_data_dict.items()]
        all_tasks = [k for k, v in train_examples_data_dict.items()]
        total_train_data_num = sum(
            [len(v[0]) for k, v in train_examples_data_dict.items()]
        )
        ...
        for cur_task in all_tasks:
            ...
            fa_dict[cur_task] = open(
                os.path.join(summary_dir, "{}_summary.log".format(cur_task)), "a+"
            )
        ...
        while True:
            ...
            if nb_tr_steps % args.gradient_accumulation_steps == 0:
                ...
                bar.update(1)
                bar.set_description(
                    "[{}] Train loss {}".format(step, round(train_loss, 3))
                )

                if (
                    args.local_rank in [-1, 0]
                    and args.log_steps > 0
                ...
                if (
                    ...
                    if global_step % 100000 == 0:
                        ...
                        last_output_dir = os.path.join(
                            args.output_dir, "checkpoint-step-{}".format(step_tag)
                        )
                        if not os.path.exists(last_output_dir):
                            os.makedirs(last_output_dir)
                        model_to_save = (
                    ...
                    if "dev_loss" in dev_dataset:
                        ...
                        eval_examples_data_dict = load_and_cache_multi_gen_data(
                            args, pool, tokenizer, "dev"
                        )
                    ...
                    for cur_task in eval_examples_data_dict.keys():
                        ...
                        logger.info(
                            "  "
                            + "***** Running ppl evaluation on [{}] *****".format(
                                cur_task
                            )
                        )
                        ...
                        for batch in tqdm(
                            eval_dataloader, total=len(eval_dataloader), desc="Eval ppl"
                        ):
                            batch = tuple(t.to(args.device) for t in batch)
                            source_ids, target_ids = batch
                            source_mask = source_ids.ne(tokenizer.pad_token_id)
                        ...
                        if eval_loss < best_loss[cur_task]:
                            ...
                            output_dir = os.path.join(
                                args.output_dir, "checkpoint-best-ppl", cur_task
                            )
                    ...
                    if args.do_eval_bleu:
                        eval_examples_data_dict = load_and_cache_multi_gen_data(
                            args, pool, tokenizer, "dev", only_src=True, is_sample=True
                        )
                        for cur_task in eval_examples_data_dict.keys():
                            if is_early_stop[cur_task]:
                                continue
                            ...
                            if dev_bleu_em > best_bleu_em[cur_task]:
                                ...
                                logger.info(
                                    "  [%d: %s] Best bleu+em: %.2f (bleu: %.2f, em: %.2f)",
                                    global_step,
                                    cur_task,
                                    dev_bleu_em,
                                    dev_bleu,
                                ...
                                output_dir = os.path.join(
                                    args.output_dir, "checkpoint-best-bleu", cur_task
                                )
                                if not os.path.exists(output_dir):
                                    os.makedirs(output_dir)
                                if args.data_num == -1 or args.always_save_model:
                                    ...
                                    logger.info(
                                        "Save the best bleu model into %s",
                                        output_model_file,
                                    )
                            else:
                                not_bleu_em_inc_cnt[cur_task] += 1
                                logger.info(
                                    "[%d %s] bleu/em does not increase for %d eval steps",
                                    global_step,
                                    cur_task,
                                    not_bleu_em_inc_cnt[cur_task],
                                )
                                if (
                                    not_bleu_em_inc_cnt[cur_task]
                                    > patience_dict[cur_task]
                                ):
                                    logger.info(
                                        "[%d %s] Early stop as bleu/em does not increase for %d eval steps",
                                        global_step,
                                        cur_task,
                                        not_bleu_em_inc_cnt[cur_task],
                                    )
    ...
    if args.do_test:
        ...
        logger.info("  Batch size = %d", args.eval_batch_size)
        eval_examples_data_dict = load_and_cache_multi_gen_data(
            args, pool, tokenizer, "test", only_src=True
        )
