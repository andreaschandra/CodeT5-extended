...
def get_cmd(
    ...
    if max_steps is None:
        ...
        cmd_str = (
            "bash exp_with_args.sh %s %s %s %d %d %d %d %d %d %d %d %d %s %s %s %d %d %d"
            % (
                task,
                sub_task,
                model_tag,
...
def run_one_exp(args):
    bs, lr, src_len, trg_len, patience, epoch = get_args_by_task_model(
        args.task, args.sub_task, args.model_tag
    )
    print("============================Start Running==========================")
    cmd_str = get_cmd(
        task=args.task,
